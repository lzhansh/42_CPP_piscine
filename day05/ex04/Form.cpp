...
void Form::beSigned(Bureaucrat &obj) {
  if (obj.getGrade() > this->_sg)
  {
    std::cout << obj.getName() << " cannot sign the <" << this->_name
              << "> because his/her grade is " << obj.getGrade() << " and required grade for signing is ["
              << this->_sg << "]." << std::endl;
  ...
  {
    std::cout << "Bureaucrat " << obj.getName() << " (Grade "<< obj.getGrade() << ") signs form targeter on <" << this->_name << ">." << std::endl;
    obj.signForm();
    this->_signed = true;
  }
  return;
